...
import {
  ...
  CreditCard, UserPlus, Briefcase, Settings as SettingsIcon, LogOut, Bell, BookOpen
} from "lucide-react";
...
const Sidebar = () => {
  ...
  // Menu items updated with Training Hub
  const menuItems = [
    ...
    <div className="w-64 bg-[#020617] h-screen fixed left-0 top-0 text-white p-6 shadow-xl flex flex-col z-50 border-r border-slate-800/50">
      ...
      <div className="mb-10 flex items-center justify-between relative">
        ...
        <div className="relative">
          ...
          {showNotif && (
            <div className="absolute left-0 mt-3 w-64 bg-white rounded-2xl shadow-2xl border border-slate-100 py-3 z-50 animate-in fade-in zoom-in duration-200 text-slate-800">
              <h4 className="px-4 pb-2 border-b text-[10px] font-black text-slate-400 uppercase tracking-widest">New Notifications</h4>
              <div className="max-h-60 overflow-y-auto">
                {notifications.length > 0 ? (
                  notifications.map((notif) => (
                    <div key={notif.id} className="p-3 border-b border-slate-50 hover:bg-slate-50 transition-colors text-left">
                      <p className="text-[11px] font-bold text-slate-800">{notif.title}</p>
                      <p className="text-[9px] text-slate-400">{notif.message}</p>
                    </div>
                  ))
                ) : (
              ...
              <Link to="/notifications" onClick={() => setShowNotif(false)} className="block text-center pt-2 text-[10px] font-black text-blue-600 uppercase hover:underline">
                View All
              </Link>
            </div>
          )}
      ...
      <div className="pt-6 border-t border-slate-800/50 mt-auto">
        <button 
          ...
          className="w-full flex items-center gap-3 p-3 rounded-xl text-slate-500 hover:bg-red-500/10 hover:text-red-400 transition-all group" 
        >
